...
import { shuffleQuestions } from '@/lib/quizzes/shuffle';
import type {
  AnswerEntry,
  Quiz,
...
export function QuizRunner({ quiz }: QuizRunnerProps) {
  ...
  // Shuffled once per attempt, in the start handler, so every pupil gets a
  // different order and the randomness never runs during server rendering.
  const [sections, setSections] = useState<QuizSectionType[]>(quiz.sections);
  ...
  const handleStart = (info: { name: string; age: string }) => {
    setParticipant(info);
    setAllAnswers({});
    setSections(shuffleQuestions(quiz.sections));
  ...
  const buildAnswerEntries = (final: Record<number, string>): AnswerEntry[] => {
    ...
    for (const section of quiz.sections) {
      for (const q of section.questions) {
        ...
        const selectedText = selectedLetter
          ? (meta.options.get(selectedLetter) ?? null)
          : null;
        entries.push({
  ...
  const handleAdvance = async (sectionAnswers: Record<number, string>) => {
    ...
    const isLast = phase.sectionIndex === sections.length - 1;
    if (!isLast) {
  ...
  if (phase.kind === 'taking') {
    ...
    return (
      <QuizSection
        ...
        isLast={phase.sectionIndex === sections.length - 1}
